...
def render_history_item(record: dict, image_url: str, on_delete=None):
    """Render one past prediction: thumbnail, badge, confidence, explanation."""
    meta = get_class_meta(record["prediction"])

    with st.container(border=True):
        col_image, col_info = st.columns([1, 1.6], gap="medium")
        ...
        with col_info:
            ...
            st.markdown(
                f'<div class="confidence-value">{record["confidence"]:.2f}%</div>',
                unsafe_allow_html=True,
            )
            created_at = record.get("created_at")
            if created_at:
                st.caption(f"Scanned on {created_at}")

            with st.expander("AI Explanation"):
                st.markdown(record.get("explanation") or "No explanation available.")
